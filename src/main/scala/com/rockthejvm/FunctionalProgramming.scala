...
object FunctionalProgramming extends App {
  ...
  val aFilteredList = List(1,2,3,4,5).filter(_ <= 3) // equivalent to above, x => x <= 3
  ...
  val anExtendedList = 0 +: aList :+ 6 // List(0,1,2,4,5,6)

  // sequences
  // Seq is a trait (an abstract type)
  val aSequence: Seq[Int] = Seq(1,2,3)  // Seq.apply(1,2,3)
  val accessedElement = aSequence(1)  // aSequence.apply(1); returns element at index 1

  // vectors (type of sequence, fast for large data; has fast access time)
  // i.e. fast Seq implementation
  ...
  // tuples = groups of values un der the same value
